 ...
 */
int func_base64_encode(const unsigned char *input, size_t input_len, char *output, size_t output_size) {
    size_t i, j;
    size_t min_size;
    ...
    if (output_size < min_size) {
        fprintf(stderr,
                "func_base64_encode: output buffer too small "
                "(need %zu, got %zu)\n",
    ...
    for (i = 0, j = 0; i < input_len;) {
        ...
        output[j++] = (consumed == 1) ? '=' : base64_table[(triple >> 6) & 0x3F];
        output[j++] = (consumed <= 2) ? '=' : base64_table[triple & 0x3F];
    }
 ...
 * @return 0 成功，-1 失败
 */
int func_base64_decode(const char *input, size_t input_len, unsigned char *output, size_t output_size,
                       size_t *out_len) {
    size_t i, j;
    size_t pad = 0;
    ...
    /* 检查输出缓冲区大小 */
    {
        ...
        if (output_size < max_dec) {
            fprintf(stderr,
                    "func_base64_decode: output buffer too small "
                    "(need %zu, got %zu)\n",
                    max_dec, output_size);
            return -1;
        }
    ...
    for (i = 0, j = 0; i < total_input;) {
        ...
        for (k = 0; k < 4; k++) {
            if (input[i] == '=') {
                ...
                if (sextet[k] < 0) {
                    fprintf(stderr,
                            "func_base64_decode: invalid base64 "
                            "character '0x%02X'\n",
        ...
        triple = (unsigned int)sextet[0] << 18 | (unsigned int)sextet[1] << 12 | (unsigned int)sextet[2] << 6 |
...
static void test_roundtrip(const char *label, const char *plaintext) {
    ...
    /* 编码 */
    ret = func_base64_encode((const unsigned char *)plaintext, strlen(plaintext), encoded, sizeof(encoded));
    if (ret < 0) {
    ...
    /* 解码 */
    ret = func_base64_decode(encoded, strlen(encoded), decoded, sizeof(decoded), &decoded_len);
    if (ret < 0) {
...
static void test_binary(void) {
    unsigned char data[] = {0x00, 0x10, 0x83, 0x10, 0x51, 0x87, 0x20, 0x92, 0x8B,
                            0x30, 0xD3, 0x8F, 0x41, 0x14, 0x93, 0x55, 0x55, 0x6A};
    char          encoded[256];
    ...
    if (decoded_len == sizeof(data) && memcmp(data, decoded, decoded_len) == 0) {
