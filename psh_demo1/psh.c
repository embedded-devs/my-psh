...
static void generate_challenge(char *output, size_t len) {
    ...
    srand(seed);
    snprintf(output, len, "PSH-%04X-%04X-%04X-%04X", rand() & 0xFFFF, rand() & 0xFFFF, rand() & 0xFFFF,
             rand() & 0xFFFF);
...
static int is_allowed_command(const char *cmd) {
    ...
    /* 白名单命令匹配 */
    if (strcmp(trimmed, "dmesg") == 0 || strcmp(trimmed, "ps") == 0 || strcmp(trimmed, "free") == 0 ||
...
static void run_portal_shell(void) {
    ...
    while (!authenticated && !feof(stdin)) {
        ...
        if (mode == MODE_LOCKED) {
            ...
            /* 检查特殊命令: debug / unlock */
            if (strcasecmp(input, "debug") == 0 || strcasecmp(input, "unlock") == 0) {
                print_debug_info();
                mode = MODE_UNLOCKING;
                continue;
            }
