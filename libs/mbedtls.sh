...
# * File name  : mbedtls.sh
# * Author     : 苏木
# * Date       : 2024-11-02
# * Description: 下载并编译安装 mbedtls v4.0.0（完整版）
# *
# * ======================================================
##
# 脚本和工程路径
...
SRC_DIR_NAME=mbedtls-4.0.0
TAR_FILE=${SCRIPT_ABSOLUTE_PATH}/${TAR_NAME}
SRC_DIR=${SCRIPT_ABSOLUTE_PATH}/${SRC_DIR_NAME}
BUILD_DIR=${SRC_DIR}/_build
INSTALL_PREFIX=${SRC_DIR}/_install
DOWNLOAD_LINK=https://github.com/Mbed-TLS/mbedtls/releases/download/mbedtls-4.0.0/mbedtls-4.0.0.tar.bz2
...
do_unzip_package() {
    ...
    if [ ! -d "${SRC_DIR}" ];then
        ...
        info "source directory already exists, skip unpack."
    fi
    success "unpack done..."
}
...
do_install() {
    ...
    # 使用 cmake install 安装标准模块（lib、头文件等）
    cdi ${BUILD_DIR}
